"""
mcp/odoo_mcp_stub.py — Odoo JSON-RPC MCP Stub (Gold Tier).
...
DRY_RUN=true (default, controlled by MCP_DRY_RUN env var)
    Logs full intent to audit_logger + Logs/<today>.json.
...
DRY_RUN=false
    ...
    Requires ODOO_URL, ODOO_DB, ODOO_USER, ODOO_PASSWORD in .env.
...
def handle_odoo_create_partner(payload: dict, dry_run: bool = True) -> dict:
    ...
    if dry_run:
        ...
        log_action(SERVER_NAME, "odoo_create_partner_dry_run", {
            ...
            "task_file": task_file,
            "dry_run": True,
            "note": "Set MCP_DRY_RUN=false + configure ODOO_* env vars for real execution.",
        })
        print(f"  [{SERVER_NAME}] DRY RUN: Would create Odoo partner")
        print(f"  [{SERVER_NAME}]   Name:    {name}")
        print(f"  [{SERVER_NAME}]   Email:   {email}")
        print(f"  [{SERVER_NAME}]   Task:    {task_file}")
        print(f"  [{SERVER_NAME}]   -> Set MCP_DRY_RUN=false to enable real Odoo calls.")
        return result
    ...
    try:
        ...
        partner_id = client.create_partner(
            name=name, email=email or None, phone=phone or None, company=company or None
        )
        result = {
            "server": SERVER_NAME,
            "action_type": "odoo_create_partner",
...
def handle_odoo_create_invoice(payload: dict, dry_run: bool = True) -> dict:
    ...
    Returns invoice_id.  To post (confirm) call execute("account.move",
    "action_post", [[invoice_id]]) separately.
    """
    task_file = payload.get("task_file", payload.get("task_name", "unknown"))
    ...
    if dry_run:
        total = sum(
            l.get("quantity", 1) * l.get("price_unit", 0)
            for l in lines
        )
        result: dict = {
            "server": SERVER_NAME,
        ...
        print(f"  [{SERVER_NAME}] DRY RUN: Would create Odoo invoice")
        print(f"  [{SERVER_NAME}]   Partner ID: {partner_id}")
        print(f"  [{SERVER_NAME}]   Lines:      {len(lines)}")
        print(f"  [{SERVER_NAME}]   Currency:   {currency}")
        print(f"  [{SERVER_NAME}]   -> Set MCP_DRY_RUN=false to enable real Odoo calls.")
        return result
    ...
    try:
        ...
        invoice_id = client.create_invoice(
            partner_id=partner_id, lines=lines, currency=currency, ref=ref or None
        )
        result = {
            "server": SERVER_NAME,
            "action_type": "odoo_create_invoice",
...
def handle_odoo_list_invoices(payload: dict, dry_run: bool = True) -> dict:
    ...
    if dry_run:
        ...
        log_action(SERVER_NAME, "odoo_list_invoices_dry_run", {
            "action_type": "odoo_list_invoices",
            "limit": limit,
            "state": f"state={state}" if state else "all",
            "task_file": task_file,
            "dry_run": True,
            "note": "Set MCP_DRY_RUN=false + configure ODOO_* env vars for real execution.",
        })
        print(f"  [{SERVER_NAME}] DRY RUN: Would list Odoo invoices")
        print(f"  [{SERVER_NAME}]   Limit:      {limit}")
        print(f"  [{SERVER_NAME}]   State:      {state or 'all'}")
        print(f"  [{SERVER_NAME}]   MoveTypes:  {move_types}")
        print(f"  [{SERVER_NAME}]   -> Set MCP_DRY_RUN=false to enable real Odoo calls.")
        return result
    ...
    try:
        ...
        invoices = client.list_invoices(domain=domain, limit=limit)
        result = {
            "server": SERVER_NAME,
            "action_type": "odoo_list_invoices",
            "dry_run": False,
